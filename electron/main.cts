...
const configuredServer = new URL(configuredServerUrl);
const desktopRendererHost = configuredServer.hostname || "127.0.0.1";
const desktopBindHost = isDev ? desktopRendererHost : "0.0.0.0";
const desktopLoopbackHost = "127.0.0.1";
const desktopServerPort = Number.parseInt(new URL(configuredServerUrl).port || "3100", 10);
...
async function prepareDesktopUrls(): Promise<void> {
  ...
  const resolvedPort = await resolveAvailablePort(desktopBindHost, activeServerPort);
  ...
  activeServerUrl = `http://${desktopLoopbackHost}:${resolvedPort}`;
...
function startBundledServer(): void {
  ...
  serverProcess = spawn(runtimeBinary, [serverEntrypoint], {
    ...
    env: {
      ...
      GEMINI_NODE_BRIDGE_HOST: desktopBindHost,
