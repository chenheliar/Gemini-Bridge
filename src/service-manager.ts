...
export class ServiceManager {
  ...
  setListeningAddress(host: string, port: number): void {
  ...
  private getLanHosts(): string[] {
    if (this.listeningHost === "127.0.0.1" || this.listeningHost === "localhost") {
      return [];
    }

    if (this.listeningHost && this.listeningHost !== "0.0.0.0" && this.listeningHost !== "::") {
    ...
    for (const networkName of Object.keys(networkInterfaces)) {
      const addresses = networkInterfaces[networkName];
      if (!addresses) {
        continue;
      }
  ...
  getNetworkEndpoints(): NetworkEndpoint[] {
    const endpoints: NetworkEndpoint[] = [
      {
        kind: "local",
        host: "127.0.0.1",
        url: `http://127.0.0.1:${this.listeningPort}/v1`,
      },
    ];

    for (const host of this.getLanHosts()) {
      endpoints.push({
        kind: "lan",
        host,
        url: `http://${host}:${this.listeningPort}/v1`,
      });
    }
  ...
  getStatus() {
    this.cookieInspection = this.inspectCurrentCookies();
    const networkEndpoints = this.getNetworkEndpoints();
    return {
      ...
      localApiBaseUrl: networkEndpoints.find((endpoint) => endpoint.kind === "local")?.url ?? null,
      lanApiBaseUrls: networkEndpoints.filter((endpoint) => endpoint.kind === "lan").map((endpoint) => endpoint.url),
      networkEndpoints,
